...
names = ['embedding_qwen3_4b', 'embedding_qwen3_0.6b', 'embedding_bge_m3']
...
def chroma_build_index(embedding_cache, chroma_persist):
...
if __name__ == "__main__":
    ...
    for type in types:
        server_path = f"{PROJECT_PATH}//data//mcp-manager//manager_server_with_task_{type}.jsonl"
        # server_path = f"{PROJECT_PATH}//data//mcp-manager//manager_server_with_task_new.jsonl"
        mcp_servers = read_jsonl_file(server_path, ManagerServer)
        for i, embedding_model in enumerate(models):
            chroma_persist = f'{PROJECT_PATH}/{DB_TYPE}/{names[i]}'
            embedding_cache = f'{embedding_path}//{names[i]}'
            if not os.path.exists(chroma_persist):
                chroma_build_index(embedding_cache, chroma_persist)
            ...
            run_grid_search(
                ...
                pass_at_k=pass_at_k,
                num_position_ratios=400,  # 位置等分数量
                num_sample_sizes=400,  # 样本大小数量
        ...
        for i, name in enumerate(names):
            srs = read_jsonl_file(
                ...
                ToolSelectionResult)
            for sr in srs:
                name = f'{sr.target_server_name}-{sr.target_tool_name}'
                cr = final_result_dict.get(name, EmbeddingCompareResult(server=sr.target_server_name,
                                                                        tool=sr.target_tool_name, res=[]))
                if sr.is_correct:
        ...
        for cr in crs:
            ...
            for res in cr.res:
                ...
                if hit:
                    ...
                    #print(f"model-{model}, rank-{rank}, score-{score}")
                else:
                    mis_match[model] += 1
