...
class GAIAPredict(MCPPredict):
    def __init__(self, max_steps=5, system_prompt=(REACT_PROMPT + SYSTEM_PROMPT).strip(), task_name="gaia"):
        super().__init__(max_steps, system_prompt, task_name)

    def evaluate_prediction(self, question: str, ground_truth: str, prediction: str, record: LLMCallRecord) -> Tuple[
        bool, Optional[str]]:
    ...
    def forward(self, **kwargs) -> dspy.Prediction:
        ...
        success = self.evaluate_prediction(question, gt, self.extract_last_answer(messages[-1][CONTENT]),
                                           manager)
